...
class TestLoadState:
    ...
    def test_loads_existing_state(self, tmp_path):
        ...
        data = {"issues": {"url1": {"last_seen_at": "2024-01-01T00:00:00Z"}}}
...
class TestSaveState:
    def test_writes_json(self, tmp_path):
        sf = tmp_path / "state.json"
        with patch.object(state_mod, "_STATE_FILE", sf):
            data = {"issues": {"url": {"last_seen_at": "2024-01-01T00:00:00Z"}}}
...
class TestGetIssueState:
    ...
    def test_returns_entry_for_known_url(self, tmp_path):
        sf = tmp_path / "state.json"
        entry = {"last_seen_at": "2024-01-01T00:00:00Z", "pr_url": "https://example.com"}
...
class TestUpdateIssueState:
    def test_creates_new_entry(self, tmp_path):
        ...
        sf.write_text(json.dumps({"issues": {}}))
        with patch.object(state_mod, "_STATE_FILE", sf):
            state_mod.update_issue_state("url1", {
                "last_seen_at": "2024-01-01T00:00:00Z", "owner": "o",
            })
        result = json.loads(sf.read_text())
        assert result["issues"]["url1"]["last_seen_at"] == "2024-01-01T00:00:00Z"
        assert result["issues"]["url1"]["owner"] == "o"

    def test_merges_updates(self, tmp_path):
        sf = tmp_path / "state.json"
        sf.write_text(json.dumps({"issues": {"url1": {
            "last_seen_at": "2024-01-01T00:00:00Z", "owner": "o",
        }}}))
        with patch.object(state_mod, "_STATE_FILE", sf):
            state_mod.update_issue_state("url1", {
                "last_seen_at": "2024-06-01T00:00:00Z", "pr_url": "pr",
            })
        result = json.loads(sf.read_text())
        assert result["issues"]["url1"] == {
