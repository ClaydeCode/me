...
def test_tick_routes_implement(mock_assigned, _fop, _ib, _dp, mock_impl):
...
@pytest.mark.parametrize("phase,handler_key,pr_url", [
...
def test_phase_routes_to_correct_handler(phase, handler_key, pr_url):
    n, patches = _patched_tick(phase, pr_url=pr_url)
    assert n == 1
    patches[handler_key].assert_called_once()


@pytest.mark.parametrize("phase", [Phase.CI_WAIT, Phase.AWAITING_MERGE])
def test_noop_phases_do_not_call_handlers(phase):
    n, patches = _patched_tick(phase, pr_url=APP_PR_URL)
